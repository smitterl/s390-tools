...
int pv_digest_ctx_update_raw(EVP_MD_CTX *ctx, const uint8_t *buf, size_t size,
			     GError **error)
{
	if (!buf || size == 0)
		return 0;
...
int pv_digest_ctx_update(EVP_MD_CTX *ctx, const GBytes *data, GError **error)
{
	const uint8_t *buf;
	size_t buf_size;
...
GBytes *pv_digest_ctx_finalize(EVP_MD_CTX *ctx, GError **error)
{
	...
	g_autofree unsigned char *digest = NULL;
...
HMAC_CTX *pv_hmac_ctx_new(GBytes *key, const EVP_MD *md, GError **error)
{
	...
	if (HMAC_Init_ex(ctx, key_data, (int)key_size, md, NULL) != 1) {
		g_set_error(error, PV_HASH_ERROR, PV_HASH_ERROR_INTERNAL,
			    "unable to create HMAC context: %s",
			    get_openssl_error());
		return NULL;
	}
	return g_steal_pointer(&ctx);
...
int pv_hmac_ctx_update_raw(HMAC_CTX *ctx, const void *buf, size_t size,
			   GError **error)
{
	if (!buf || size == 0)
		return 0;
	...
	if (HMAC_Update(ctx, buf, size) != 1) {
		g_set_error(error, PV_HASH_ERROR, PV_HASH_ERROR_INTERNAL,
			    "unable to add data to HMAC context: %s",
			    get_openssl_error());
		return -1;
	}
	return 0;
