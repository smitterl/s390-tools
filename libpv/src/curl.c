			     ...
			     void *userdata)
{
   ...
   */
	...
	data->buffer =
		g_byte_array_append(buffer, (uint8_t *)ptr, (uint)actual_size);
			  ...
			  GError **err)
{
	...
	if (rc != CURLE_OK) {
		g_set_error(err, PV_CERT_ERROR, PV_CERT_ERROR_DOWNLOAD_FAILED,
...
curl_err:
	g_set_error(err, PV_CERT_ERROR, PV_CERT_ERROR_CURL_INIT_FAILED,
