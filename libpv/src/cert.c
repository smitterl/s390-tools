...
static GHashTable *cached_crls = NULL;
...
/* Reads only the first X509 certificate in a PEM-formatted certificate */
X509 *pv_x509_from_pem_der_data(GBytes *data, GError **error)
{
	...
	bio = BIO_new_mem_buf(data_buffer, (int)data_size);
	if (!bio) {
		g_set_error(error, PV_CERT_ERROR,
			    PV_CERT_ERROR_READ_CERTIFICATE,
			    _("unable to read certificate"));
...
static gboolean certificate_uses_elliptic_curve(EVP_PKEY *key, int nid,
						GError **error)
{
...
EVP_PKEY *pv_read_ec_pubkey_cert(X509 *cert, int nid, GError **error)
{
	...
	if (!certificate_uses_elliptic_curve(ret, nid, error)) {
		g_prefix_error(
			error,
			_("Host-key document does not use a elliptic EC curve"));
		return NULL;
	}
				  ...
				  GError **error)
{
	...
	for (GSList *iterator = certs_with_path; iterator;
		...
		host_key = pv_read_ec_pubkey_cert(cert, nid, error);
		if (!host_key)
			return NULL;
					 ...
					 GError **error)
{
	...
	for (const char *const *iterator = cert_paths;
		...
		ret = g_slist_append(ret,
				     pv_x509_with_path_new(cert, cert_path));
	}
	if (!ret) {
		g_set_error(error, PV_CERT_ERROR,
			    PV_CERT_ERROR_READ_CERTIFICATE,
...
static gboolean cache_crl(const X509_NAME *name, X509_CRL *crl)
{
	...
	X509_NAME_print_ex(key_bio, name, 0, XN_FLAG_ONELINE);
	len = BIO_get_mem_data(key_bio, &key);
	if (len < 0)
		g_abort();
	/* So BIO_free() leaves key alone and does not free it */
	BIO_set_close(key_bio, BIO_NOCLOSE);
...
static STACK_OF_X509_CRL *crls_download_cb(const X509_STORE_CTX *ctx,
					   const X509_NAME *nm)
{
 ...
 * command line.
 */
static STACK_OF_X509_CRL *crls_cb(const X509_STORE_CTX *ctx,
				  const X509_NAME *nm)
{
...
static const char *get_first_dp_url(DIST_POINT *dp)
{
	...
	for (int i = 0; i < sk_GENERAL_NAME_num(general_names); i++) {
		...
		/* Make sure that uri_str is null-terminated as in general it
	 	 ...
	 	 */
		uri_str = g_strndup(uri_data, (gsize)uri_data_len);
...
#define DEFINE_GSLIST_MAP(t2, t1)                                              \
	...
	G_GNUC_UNUSED static GSList *g_slist_map_##t2##_##t1(                  \
		const GSList *list, g_slist_map_func_##t2##_##t1 func,         \
		GError **error)                                                \
	{                                                                      \
		...
		for (const GSList *iterator = list; iterator;                  \
			...
			if (!new_value)                                        \
				return NULL;                                   \
			ret = g_slist_append(ret,                              \
					     g_steal_pointer(&new_value));     \
		}                                                              \
		return g_steal_pointer(&ret);                                  \
	}

#define DEFINE_GSLIST_TO_STACK(t1)                                             \
	G_GNUC_UNUSED static STACK_OF(t1) *                                    \
		g_slist_to_stack_of_##t1(GSList **list)                        \
	{                                                                      \
		g_assert(list);                                                \
		...
		for (GSList *iterator = *list; iterator;                       \
		     iterator = iterator->next) {                              \
			if (sk_##t1##_push(                                    \
				    ret, g_steal_pointer(&iterator->data)) ==  \
			    0)                                                 \
				g_abort();                                     \
		}                                                              \
...
static X509 *get_cert(const PvX509WithPath *cert_with_path,
...
static int load_crls_to_store(X509_STORE *store, const char *const *crl_paths,
			      gboolean err_out_empty_crls, GError **error)
{
	for (const char *const *iterator = crl_paths;
	     iterator != NULL && *iterator != NULL; iterator++) {
		...
		count = X509_load_crl_file(lookup, crl_path,
					   X509_FILETYPE_ASN1);
		if (count == 1)
			continue;
			   ...
			   GError **error)
{
	...
	if (root_ca_path) {
		...
		/* try to load a CRL for the root CA */
		X509_load_crl_file(lookup, root_ca_path, X509_FILETYPE_PEM);
	} else {
...
STACK_OF_X509 *pv_get_x509_stack(const GSList *x509_with_path_list)
{
	...
	certs = g_slist_map_PvX509WithPath_X509(x509_with_path_list, get_cert,
						&error);
...
int pv_init_store_ctx(X509_STORE_CTX *ctx, X509_STORE *trusted,
		      STACK_OF_X509 *chain, GError **error)
{
	if (1 != X509_STORE_CTX_init(ctx, trusted, NULL, chain)) {
		g_set_error(error, PV_CERT_ERROR, PV_CERT_ERROR_INTERNAL,
				    ...
				    GError **error)
{
	...
	if (0 != pv_init_store_ctx(ctx, trusted, chain, error))
		return NULL;
...
int pv_store_set_verify_param(X509_STORE *store, GError **error)
{
	 ...
	 */
	if (X509_VERIFY_PARAM_set_purpose(
		    param, X509_PURPOSE_ANY | X509_TRUST_DEFAULT) != 1)
		goto error;
...
static int x509_name_entry_get_data0(X509_NAME_ENTRY *entry,
				     const guchar **data, size_t *data_len)
{
...
static int x509_name_get_data0_by_NID(X509_NAME *name, int nid,
				      const guchar **data, size_t *data_len)
{
...
static gboolean x509_name_data_by_nid_equal(X509_NAME *name, int nid,
					    const char *y)
{
...
static X509_NAME *x509_name_reorder_attributes(const X509_NAME *name,
					       const int nids[],
					       size_t nids_len)
{
	...
	for (size_t i = 0; i < nids_len; i++) {
		...
		lastpos = X509_NAME_get_index_by_NID((X509_NAME *)name, nid,
						     lastpos);
		if (lastpos == -1)
			return NULL;
...
X509_NAME *pv_c2b_name(const X509_NAME *name)
{
	...
	broken_name =
		x509_name_reorder_attributes(name, nids, G_N_ELEMENTS(nids));
	if (broken_name)
 ...
 *   1 when expired
 */
static int check_validity_period(const ASN1_TIME *not_before,
					  ...
					  const X509_NAME_ENTRY *y)
{
	...
	return OBJ_cmp(x_obj, y_obj) == 0 &&
	       memcmp(ASN1_STRING_get0_data(x_data),
		      ASN1_STRING_get0_data(y_data), (unsigned long)x_len) == 0;
}
				      ...
				      GError **error)
{
	...
	if (!pkey) {
		g_set_error(error, PV_CERT_ERROR, PV_CERT_ERROR_NO_PUBLIC_KEY,
			    _("no public key"));
		return -1;
	}
...
static int check_host_key_issued(X509 *cert, X509 *issuer, GError **error)
{
	...
	if (check_signature_algo_match(X509_get0_pubkey(issuer), cert, error) <
	    0)
		return -1;
...
static gboolean is_cert_revoked(X509 *cert, X509_CRL *crl)
{
	...
	rc = X509_CRL_get0_by_serial(
		crl, &revoked, (ASN1_INTEGER *)X509_get0_serialNumber(cert));
	if (rc == 0)
		return FALSE;
...
static int check_crl_issuer(X509_CRL *crl, X509 *issuer, GError **error)
{
	...
	if (!own_X509_NAME_equal(issuer_subject, crl_issuer)) {
		g_autofree char *issuer_subject_str =
			X509_NAME_oneline(issuer_subject, NULL, 0);
		g_autofree char *crl_issuer_str =
			X509_NAME_oneline(crl_issuer, NULL, 0);
	...
	akid = X509_CRL_get_ext_d2i(crl, NID_authority_key_identifier, NULL,
				    NULL);
	if (akid && X509_check_akid(issuer, akid) != X509_V_OK) {
		g_set_error(error, PV_CERT_ERROR,
			    PV_CERT_ERROR_SKID_AKID_MISMATCH,
...
int pv_check_chain_parameters(const STACK_OF_X509 *chain, GError **error)
{
	...
	if (len < 2) {
		g_set_error(
			error, PV_CERT_ERROR, PV_CERT_ERROR_INTERNAL,
			"there must be at least on root and one leaf certificate in the chain of trust");
		return -1;
	}
	...
	if (!ca_x509_subject) {
		g_set_error(error, PV_CRYPTO_ERROR, PV_CRYPTO_ERROR_INTERNAL,
			    "subject of the root CA cannot be retrieved");
		return -1;
	}
	...
	if (!ca_subject) {
		g_set_error(error, PV_CRYPTO_ERROR, PV_CRYPTO_ERROR_INTERNAL,
			    "subject name of the root CA cannot be retrieved");
		return -1;
	}
						...
						GError **error)
{
	...
	for (int i = 0; i < sk_X509_CRL_num(ret); i++) {
		 ...
		 * warning.
		 */
		if (pv_check_crl_valid_for_cert(crl, cert, verify_flags,
						error) < 0) {
			g_assert(error);
 ...
 * Error, if there is not exactly one IBM signing key.
 */
static STACK_OF_X509 *get_ibm_singing_certs(STACK_OF_X509 *certs,
					    GError **error)
{
...
static gboolean download_crls(X509_STORE *trusted,
			      ...
			      GError **error)
{
	 ...
	 * line.
	 */
	downloaded_ibm_signing_crls =
		pv_try_load_crls_by_certs(host_key_certs_with_path);
	...
	for (int i = 0; i < sk_X509_CRL_num(downloaded_ibm_signing_crls); i++) {
		X509_CRL *crl =
			sk_X509_CRL_value(downloaded_ibm_signing_crls, i);
...
gboolean pv_verify_host_key_doc(PvCertWithPathList *host_key_certs_with_path,
				X509_STORE *trusted,
				STACK_OF_X509 *untrusted_certs, gboolean online,
				GError **error)
{
	 ...
	 * actual host-key documents.
	 */
	ibm_signing_certs = get_ibm_singing_certs(untrusted_certs, error);
	 ...
	 * NOTE: Currently there is only one signing-key allowed previously
	 */
	for (int i = 0; i < sk_X509_num(ibm_signing_certs); i++) {
		 ...
		 */
		ibm_signing_crls = pv_store_ctx_find_valid_crls(
			ctx, ibm_signing_cert, error);
		if (!ibm_signing_crls) {
		...
		ibm_z_pair = pv_x509_pair_new_take(&ibm_signing_cert,
						   &ibm_signing_crls);
	 ...
	 * lists.
	 */
	for (GSList *iterator = host_key_certs_with_path; iterator;
	     iterator = iterator->next) {
		...
		if (pv_verify_host_key(host_key, ibm_z_pairs, flags,
				       PV_CERTS_SECURITY_LEVEL, error) < 0) {
	 ...
	 */
	for (GSList *iterator = ibm_z_pairs; iterator;
	     iterator = iterator->next) {
		const PvX509Pair *ibm_z_pair = iterator->data;

		if (pv_verify_cert(ibm_z_pair->cert, ctx, error) < 0)
			return -1;
		if (pv_check_chain_parameters(X509_STORE_CTX_get0_chain(ctx),
					      error) < 0)
			return -1;
		/* re-init ctx for the next verification */
		X509_STORE_CTX_cleanup(ctx);
		if (0 !=
		    pv_init_store_ctx(ctx, trusted, untrusted_certs, error))
			return -1;
	}
	return 0;
}

gboolean pv_verify_host_key_doc_by_path(const char *const *host_key_paths,
					const char *optional_root_ca_path,
					const char *const *crl_paths,
					const char *const *untrusted_cert_paths,
					gboolean online, GError **error)
{
	g_autoslist(PvX509WithPath) untrusted_certs_with_path = NULL,
				    host_key_certs = NULL;
	 ...
	 */
	trusted = pv_store_setup(optional_root_ca_path, crl_paths,
				 untrusted_cert_paths, error);
	if (!trusted)
		return -1;
	...
	untrusted_certs_with_path =
		pv_load_certificates(untrusted_cert_paths, error);
	if (!untrusted_certs_with_path)
		return -1;
	/* Convert to STACK_OF(X509) */
	untrusted_certs = pv_get_x509_stack(untrusted_certs_with_path);

	return pv_verify_host_key_doc(host_key_certs, trusted, untrusted_certs,
				      online, error);
}
